...
async def load_from_to(
    ...
    Modified to handle restricted chats by:
    1. Attempting to join private chats if invite link provided
    2. Storing chat access credentials
    3. Handling exceptions for restricted access
    """
    from_to_dict = {}
    
    async def handle_chat_access(peer):
        try:
            # Try getting chat ID directly first
            chat_id = await get_id(client, peer)
            return chat_id
        except Exception:
            # If failed, check if it's an invite link
            if isinstance(peer, str) and ('t.me/+' in peer or 't.me/joinchat/' in peer):
                try:
                    # Try joining the chat
                    updates = await client(ImportChatInviteRequest(
                        hash=peer.split('/')[-1]
                    ))
                    # Get the chat ID after joining
                    if hasattr(updates, 'chats') and updates.chats:
                        return updates.chats[0].id
                except Exception as e:
                    logging.error(f"Failed to join chat via invite link: {e}")
            return None
    ...
    for forward in forwards:
        ...
        src = await handle_chat_access(forward.source)
        if not src:
            logging.warning(f"Could not access source chat: {forward.source}")
            continue
            
        dest_ids = []
        for dest in forward.dest:
            dest_id = await handle_chat_access(dest)
            if dest_id:
                dest_ids.append(dest_id)
            else:
                logging.warning(f"Could not access destination chat: {dest}")
