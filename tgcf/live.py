...
async def new_message_handler(event: Union[Message, events.NewMessage]) -> None:
    """Process new incoming messages with restricted chat handling."""
        ...
        for d in dest:
            try:
                ...
                if hasattr(message, 'media') and message.media:
                    ...
                    tm.new_file = file_data

                fwded_msg = await send_message(d, tm)
                st.stored[event_uid].update({d: fwded_msg.id})

            except Exception as e:
                logging.error(f"Failed to forward to {d}: {str(e)}")
                try:
                    fwded_msg = await tm.client.send_message(
                        d,
                        message.text if message.text else None,
                        file=tm.new_file if hasattr(tm, 'new_file') else None,
                        formatting_entities=tm.text_entities,
                    )
                    st.stored[event_uid].update({d: fwded_msg.id})
                except Exception as e2:
                    logging.error(f"Alternative sending also failed for {d}: {str(e2)}")
                continue

        tm.clear()

    except Exception as err:
        logging.exception(f"Error processing message: {err}")
